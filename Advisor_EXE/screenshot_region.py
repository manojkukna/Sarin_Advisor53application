...
def screenshot(
    ...
    screenshot = pyautogui.screenshot(region=(710, 130, 200, 80))
    screenshot = cv2.cvtColor(np.array(screenshot), cv2.COLOR_RGB2BGR)
    for pawn in pyautogui.locateAllOnScreen('pawn.png', confidence=0.5):
        # draw rectangle around the object
        cv2.rectangle(screenshot,(pawn.left, pawn.top),(pawn.left + pawn.width, pawn.top + pawn.height),
            (0, 0, 255),2)
