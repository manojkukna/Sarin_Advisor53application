...
#  ////////progam mame Advisor 17/01/2023 12:23 pm /////////////////           Planning_Templates
#  ////////progam mame Advisor 17/01/2023 02:11 PM /////////////////           def  PEAR
...
def MARQUISE(image):
    Planning_Templates(image="Planning_Templates.PNG")
    MARQUISE = locate_leftClick(image=image, confide_=0.80, clicks_=1)
    time.sleep(1)
    sawless(image="sawless.PNG")
    ResultComparison(image="ResultComparison.PNG")
    group_Name_selet(image="group_Name.PNG")
    tag_click(image="Tag_click.PNG", dragRel=200, okPNG="ok.PNG")
    print(f"{image}/{MARQUISE}", "MARQUISE", time_())
    time_difference_seconds(start_time=start_time)
