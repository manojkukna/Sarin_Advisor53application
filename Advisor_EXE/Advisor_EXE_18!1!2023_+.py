...
def python_text_detection():
    ...
    print("11111111", time_())
    img = pyautogui.screenshot(region=(710, 130, 140, 80))

    output1 = f'{pytesseract.image_to_string(img)}'

    output = output1[0:5]
    print("str",type( output))

    result1 = float(output)
    print("float",type(  result1))
    result = float(result1 )

    while 0 > result or result == None  :
