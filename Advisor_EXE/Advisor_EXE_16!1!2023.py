...
def locate_rightClick(image, confide_, clicks_):
    moveTo_ = none_valu(image=image, confide_=confide_)
    pyautogui.moveTo(moveTo_, duration=0.1)
...
#     global current
#     locsan = "./PNGFIL"
#     opne_text = open(f"{locsan}/{txt}", "r")
#     for line in (opne_text.readlines()[-1:]):
#         current = line
#         # print(current,end="")
#         opne_text_append.append(current)
#         print("def opne_tex",opne_text_append)
#         print("def opne_tex",len(opne_text_append))
#         opne_text.close()
#     return current
...
def sawless(image):
    ...
    while float(current) > 0.0039 and opne_text_ < 5 :
        ...
        current = current_valu[-1]
        print(current_valu, " current", current_valu[-1], time_())
...
def Group_Name_Sort_Confirm(image):
    ...
    while moveTo_ is None:
          ...
          moveTo_ = pyautogui.locateCenterOnScreen(f"{locsan}/11eerl Group_Name_Sort.PNG", confidence=0.80)
          time.sleep(1)
          pyautogui.moveTo(moveTo_, duration=0.10)
    else:
         ...
         if moveTo_ == moveTo_Group:
            time.sleep(1)
            pyautogui.moveTo(moveTo_, duration=0.10)
            print(f'{ Group }', "222Group_Name_Sort.PNG", time_())
    return moveTo_
...
def group_Name_selet(image):
    ...
    print(f'{image}current_valu:  {current_valu[-1] }    sawless_count  : {sawless_count}', "     333Group_Name_Sort.PNG", time_())
    if sawless_count == 1:  #(0.0039)  > float(current_valu[-1]) and
        Min_polish_pant(image="group_Name.PNG",image2="Group_Name_Sort.PNG")
    else:
       print(f"{image}", "44444eeeee", time_())
       solution_selet = locate_leftClick(image=image, confide_=0.80, clicks_=1)
       print(f"{image}", "55555eeeee", time_())
       pyautogui.dragRel(0, 50,)
       pyautogui.click(clicks=2)
       time.sleep(1)
       print(solution_selet, sawless_count,python_text_detection(), "group_Name_selet", time_())
