...
def opne_text(txt):
    ...
    for line in (opne_text.readlines()[-1:]):
        ...
        opne_text.close()
    return current
def open_text_clear(txt="opne_txt.txt"):
    locsan = "./PNGFIL"
    opne_text = open(f"{locsan}/{txt}", "r")
    print("open_text",opne_text)
    with open(f"{locsan}/{txt}", 'w') as f:
         opne_text = f.write("")
         print("open_text_clear",opne_text)
...
def sawless(image):
    ...
    all_plan_count = 0
    sawless_count = 0
    while float(current) > 0.0039 and opne_text_ < 5 or float(current) == None:
        ...
        print(current_valu, " current", time_())
...
def group_Name_selet(image):
    ...
    if (0.0039) > float(opne_text(txt="opne_txt.txt")) and sawless_count == 1:
       ...
       pyautogui.click(clicks=2)
       time.sleep(1)
       print(solution_selet, sawless_count,opne_text(txt="opne_txt.txt"), "group_Name_selet", time_())
