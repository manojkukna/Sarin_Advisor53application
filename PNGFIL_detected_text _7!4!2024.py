...
directory = r'C:\Users\ADMIN\PycharmProjects\pythonProject5  uiautomation  WindowControl\Advisor_EXE\PNGFIL\phone_numbers_png'
...
for root, dirs, files in os.walk(directory):
    for filename in files:
        if filename.endswith(('.jpg', '.jpeg', '.png', '.bmp')):  # Consider only image files
            image_path = os.path.join(root, filename)
            extracted_text = perform_text_detection(image_path)
            # print(f"Text extracted from {image_path}:\n{extracted_text}\n")
            phone_numbers1 = re.findall(r'\+\d{2} \d{5} \d{5}', extracted_text)

            all_phone_numbers.extend(phone_numbers1)
            ...
            data = {'Phone numbers': all_phone_numbers}
